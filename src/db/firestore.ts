...
/** コミュニティ一覧を購読 */
export function subscribeToCommunities(callback: (list: FirestoreCommunity[]) => void) {
  const ref = collection(db, 'communities');
  return onSnapshot(ref, (snapshot) => {
    const list = snapshot.docs.map((d) => ({
      ...d.data(),
      id: d.id,
    }) as FirestoreCommunity);
    callback(list);
  });
}

/** コミュニティ作成 */
export async function createCommunityFirestore(commData: Omit<FirestoreCommunity, 'id' | 'memberCount' | 'memberIds'> & { createdBy: string }): Promise<FirestoreCommunity> {
  const ref = collection(db, 'communities');
  const docRef = await addDoc(ref, {
    ...commData,
    memberCount: 1,
    memberIds: [commData.createdBy],
  });
  return {
    ...commData,
    id: docRef.id,
...
/** コミュニティ参加 */
export async function joinCommunityFirestore(communityId: string, userId: string): Promise<void> {
...
/** コミュニティ退会 */
export async function leaveCommunityFirestore(communityId: string, userId: string): Promise<void> {
  const ref = doc(db, 'communities', communityId);
  await updateDoc(ref, {
...
/** コミュニティメッセージの購読 */
export function subscribeToCommunityMessages(communityId: string, callback: (messages: FirestoreCommunityMessage[]) => void) {
  ...
  return onSnapshot(q, (snapshot) => {
    const messages = snapshot.docs.map((d) => ({
      ...
      id: d.id,
      createdAt: d.data().createdAt?.toDate?.() || new Date(),
    }) as FirestoreCommunityMessage);
    callback(messages);
  });
}

/** コミュニティメッセージ送信 */
export async function sendCommunityMessageFirestore(communityId: string, userId: string, content: string): Promise<void> {
  const ref = collection(db, 'communities', communityId, 'messages');
  await addDoc(ref, {
    userId,
    content,
